...
class Layer(object):
    ...
    def pre_activation(self,x):
        ...
        #############################################################################
        # START OF YOUR CODE 
        z = np.dot(np.matrix(self.W),np.matrix(x)) + np.matrix(self.b)
        return z
    ...
    def activation(self):
        ...
        #############################################################################
        # START OF YOUR CODE 
            a= np.divide(np.exp(self.z),(np.exp(self.z)+1.0))
            return a
    ...
    def back_propagation(self,x ,delta_l, next_layer = None):
        ...
        if next_layer is None:
            self.grad_W = delta_l*x.T
            self.grad_b = delta_l
            delta_l = delta_l/(self.a*(1-self.a))
        else:
            if self.activation_type == 'sigmoid':
                #delta_l = delta_l*next_layer.a*(1-next_layer.a)*next_layer.W
                #self.grad_W = np.multiply(delta_l.T,np.multiply(self.a,(1-self.a))).dot(x.T)
                delta_l = np.multiply(np.dot(np.matrix(next_layer.W).T,np.matrix(delta_l)),np.multiply(self.a,(1-self.a)))                
                self.grad_W = np.dot(delta_l, x.T)
                self.grad_b = delta_l
            elif self.activation_type == 'ReLU':
                #delta_l = delta_l*self.d_ReLU(next_layer.z)*next_layer.W
                delta_l = np.multiply(np.dot(np.matrix(next_layer.W).T,np.matrix(delta_l)),self.d_ReLU(self.z))
                self.grad_W = np.dot(delta_l, x.T)
                self.grad_b = delta_l  
...
class MultiLayerNet(object):
   ...
   A fully-connected neural network with an arbitrary number of hidden layers, each with
    ...
    def back_propagation(self,x, y):
        ...
        delta_l = layer.a-y
        if n==1:
            return layer.back_propagation(x ,delta_l)
        else: 
            delta_l = layer.back_propagation(self.layers[-2].a ,delta_l)
            for i in range(len(self.layers)-2,-1,-1):
                layer = self.layers[i]
                if i == 0:
                    
                    delta_l = layer.back_propagation(x ,delta_l, self.layers[i+1])
                else:
                    delta_l = layer.back_propagation(self.layers[i-1].a ,delta_l, self.layers[i+1])  
